...
const filters: Array<{ value: AutomatonType | 'ALL'; label: string; color: string }> = [
  ...
  { value: 'DFA', label: 'DEA', color: 'bg-blue-50 text-blue-700 hover:bg-blue-100' },
  { value: 'NFA', label: 'NEA', color: 'bg-purple-50 text-purple-700 hover:bg-purple-100' },
  { value: 'PDA', label: 'PDA', color: 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' },
  { value: 'TM', label: 'TM', color: 'bg-orange-50 text-orange-700 hover:bg-orange-100' },
]

const completionFilters: Array<{ value: 'ALL' | 'COMPLETED' | 'INCOMPLETE'; label: string; color: string }> = [
  { value: 'ALL', label: 'Alle', color: 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' },
  { value: 'INCOMPLETE', label: 'Nicht abgeschlossen', color: 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100' },
...
<template>
  <div class="flex-1 flex flex-col h-full bg-gradient-to-br from-zinc-50 via-white to-zinc-50 overflow-hidden">
    <!-- Hero Header -->
    <div class="bg-indigo-600 text-white px-8 py-6 flex-shrink-0 relative">
      <button
    ...
    <!-- Filter Tabs -->
    <div class="px-8 py-4 border-b border-zinc-200 bg-white flex-shrink-0">
          ...
          <button
            ...
            :class="
              selectedFilter === filter.value
                ...
                : 'bg-white text-zinc-400 border-transparent hover:text-zinc-600'
            "
          >
            {{ filter.label }}
    ...
    <div class="flex-1 overflow-y-auto px-8 py-6">
      <div class="max-w-4xl mx-auto space-y-3">
        <div
          ...
          class="group relative bg-white rounded-xl border border-zinc-200 hover:border-blue-300 hover:shadow-lg transition-all duration-200 cursor-pointer overflow-hidden"
        >
          <!-- Completed Glow -->
          <div
            v-if="isExerciseCompleted(exercise.id)"
            class="absolute inset-0 bg-gradient-to-r from-green-50/50 to-emerald-50/50 pointer-events-none"
          ></div>

          <div class="relative flex items-center gap-4 px-5 py-4">
            <!-- Completion Status -->
            <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center"
              :class="isExerciseCompleted(exercise.id) ? 'bg-green-100' : 'bg-zinc-100'"
            >
              <CheckCircle2
                v-if="isExerciseCompleted(exercise.id)"
                class="w-6 h-6 text-green-500"
              />
              <Minus v-else class="w-5 h-5 text-zinc-300" />
            </div>

            <!-- Exercise Info -->
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="text-sm font-bold text-zinc-900 truncate group-hover:text-blue-700 transition-colors">
                  {{ exercise.title }}
                </h3>
              </div>
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              ...
              <!-- Test Case Count -->
              <span class="px-2 py-1 rounded-full bg-zinc-100 text-zinc-500 text-[10px] font-bold">
      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center relative">
          ...
          <div class="w-14 h-14 rounded-full bg-indigo-100 flex items-center justify-center mx-auto mb-4">
            <FolderOpen class="w-7 h-7 text-indigo-600" />
          </div>
          <h2 class="text-lg font-bold text-zinc-900 mb-1">Gespeicherter Fortschritt</h2>
          <p class="text-sm text-zinc-500 mb-6">Es gibt einen gespeicherten Stand für diese Aufgabe. Möchtest du fortfahren oder neu starten?</p>
          <div class="flex gap-3">
            <button
              @click="beginExerciseFresh(pendingExerciseId!)"
              class="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-zinc-300 hover:bg-zinc-50 text-zinc-700 rounded-lg text-sm font-semibold transition-colors"
            >
